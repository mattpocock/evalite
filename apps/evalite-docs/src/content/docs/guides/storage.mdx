...
### Using `libsql` for SQLite Storage

LibSQL is a fork of SQLite, you can use it with their `better-sqlite3` compatible client library `libsql`.
Configure Evalite to use it by installing the required peer dependency:

```bash
pnpm add -D libsql
```

Then configure Evalite to use `libsql` storage:
...
export default defineConfig({
  storage: () =>
    createSqliteStorage({
      ...
      filepath: "./evalite.db",
      libSqlOptions: {
        /** other libsql config options */
      },
    }),
});
```

See [libsql](https://www.npmjs.com/package/libsql) for more information.
