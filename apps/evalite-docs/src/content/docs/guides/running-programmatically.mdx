...
Redirect test output to a custom writable stream:

```typescript
import { createWriteStream } from "fs";

const logStream = createWriteStream("./test-output.log");

await runEvalite({
  path: undefined,
  cwd: process.cwd(),
  mode: "run-once-and-exit",
  testOutputWritable: logStream,
});
```
...
## Migration from `runVitest`

If you were previously using `runVitest`, you can simply rename it to `runEvalite`. The API is identical:
...
> [!NOTE]
>
> `runVitest` is still available as a deprecated alias for backward compatibility, but it will be removed in a future version.
