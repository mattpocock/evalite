...
Specify extra file globs that trigger eval reruns in watch mode. This overrides any `watchFiles` setting in `evalite.config.ts`:

```typescript
await runEvalite({
  mode: "watch-for-file-changes",
  watchFiles: ["src/**/*.ts", "prompts/**/*", "data/**/*.json"],
});
```
...
> [!TIP] > `watchFiles` globs are evaluated relative to the `cwd` you pass (or `process.cwd()` if unspecified).
> If your watched files live outside the default working directory, point `cwd` at the project root so Vitest can see those changes.
