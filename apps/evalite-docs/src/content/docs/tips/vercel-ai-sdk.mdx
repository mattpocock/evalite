...
Combine cache with [`trialCount`](/api/evalite#optstrialcount) to solve this - each trial busts the cache and runs fresh, ensuring you get multiple samples while still benefiting from caching during development:
