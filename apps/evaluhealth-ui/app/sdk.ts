...
export const getMenuItems = async (fetchOpts?: {
  signal?: AbortSignal;
}): Promise<Evaluhealth.SDK.GetMenuItemsResult> => {
  if (isStaticMode()) {
    return safeFetch<Evaluhealth.SDK.GetMenuItemsResult>(
...
): Promise<Evaluhealth.SDK.GetEvalByNameResult> => {
  if (isStaticMode()) {
    const sanitized = sanitizeFilename(name);
    return safeFetch<Evaluhealth.SDK.GetEvalByNameResult>(
...
export const getResult = async (
  ...
  fetchOpts?: { signal?: AbortSignal }
): Promise<Evaluhealth.SDK.GetResultResult> => {
  if (isStaticMode()) {
    const sanitized = sanitizeFilename(opts.evalName);
    return safeFetch<Evaluhealth.SDK.GetResultResult>(
