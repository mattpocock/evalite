...
export interface EvaluhealthRunnerOptions {
...
export class EvaluhealthRunner {
  private opts: EvaluhealthRunnerOptions;
  private state: Evaluhealth.ServerState = { type: "idle" };
  private didLastRunFailThreshold: "yes" | "no" | "unknown" = "unknown";
  private collectedResults: Map<string, Evaluhealth.Result> = new Map();
  private eventQueue: Promise<void> = Promise.resolve();

  constructor(opts: EvaluhealthRunnerOptions) {
    this.opts = opts;
  }

  getState(): Evaluhealth.ServerState {
    return this.state;
  }
  ...
  getAllScores(): Evaluhealth.Score[] {
    return Array.from(this.collectedResults.values()).flatMap(
      (result) => result.scores
    );
  }

  getSuccessfulResults(): Evaluhealth.Result[] {
    return Array.from(this.collectedResults.values()).filter(
      (result) => result.status === "success"
    );
  }

  getScoresForModule(moduleId: string): Evaluhealth.Score[] {
  ...
  updateState(state: Evaluhealth.ServerState) {
    this.state = state;
    ...
    switch (this.state.type) {
      case "running":
        switch (event.type) {
          ...
          case "RESULT_STARTED":
            {
              const run: Evaluhealth.Storage.Entities.Run = this.state.runId
                ? (
