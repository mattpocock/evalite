...
export const renderers = {
  title: () => {
    return c.magenta(c.bold("EVALUHEALTH"));
  },
...
export function renderMaybeEvaluhealthFile(input: unknown) {
  if (EvaluhealthFile.isEvaluhealthFile(input)) {
    return input.path;
  }
      ...
      columns:
        result.renderedColumns.length > 0
          ? result.renderedColumns.map((col) => ({
              label: col.label,
              value: renderMaybeEvaluhealthFile(col.value),
            }))
          : [
              {
                label: "Input",
                value: renderMaybeEvaluhealthFile(result.input),
              },
              {
                label: "Output",
                value: renderMaybeEvaluhealthFile(result.output),
              },
            ],
      score:
...
export function renderTask(opts: {
  ...
  result: {
    filePath: string;
    status: Evaluhealth.ResultStatus;
    scores: Evaluhealth.Score[];
...
export function renderServeModeFinalMessage(
  ...
  logger.log(
    withLabel(
      "blue",
      "INFO",
      "Tests won't re-run on file changes. Re-run evaluhealth serve to try again."
    )
  );
  logger.log(
    BADGE_PADDING +
