...
import { EvaluhealthRunner } from "./reporter/EvaluhealthRunner.js";
...
export interface EvaluhealthReporterOptions {
  isWatching: boolean;
  port: number;
  logNewState: (event: Evaluhealth.ServerState) => void;
...
export default class EvaluhealthReporter implements Reporter {
  private opts: EvaluhealthReporterOptions;
  private runner: EvaluhealthRunner;
  private ctx!: Vitest;
  private start!: number;

  constructor(opts: EvaluhealthReporterOptions) {
    this.opts = opts;
    this.runner = new EvaluhealthRunner({
  ...
  onTestCaseAnnotate(testCase: TestCase, annotation: TestAnnotation): void {
    ...
    if (!data) {
      // Not an evaluhealth annotation - ignore
      return;
    }
  ...
  onTestCaseResult(test: TestCase): void {
    ...
    if (!resultStartedAnnotation) {
      // No evaluhealth annotations at all - not an evaluhealth test
      return;
    }
  ...
  protected printAnnotations(
    ...
    // Evaluhealth uses annotations internally for reporter communication.
    // Users cannot add custom annotations via the Evaluhealth API,
