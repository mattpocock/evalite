...
const executeTask = async <TInput, TOutput, TVariant = undefined>(
  task: Evaluhealth.Task<TInput, TOutput, TVariant>,
  input: TInput,
  variant: TVariant
): Promise<TOutput> => {
...
const runTask = async <TInput, TOutput, TExpected, TVariant = undefined>(
  opts: {
    ...
    Evaluhealth.RunnerOpts<TInput, TOutput, TExpected, TVariant>,
...
export const evaluhealth = <TInput, TOutput, TExpected = TOutput>(
  evalName: string,
  opts: Evaluhealth.RunnerOpts<TInput, TOutput, TExpected>
) => registerEvaluhealth(evalName, opts);

evaluhealth.skip = <TInput, TOutput, TExpected>(
  evalName: string,
  opts: Evaluhealth.RunnerOpts<TInput, TOutput, TExpected>
...
evaluhealth.each = <TVariant>(
  variants: Array<{ name: string; input: TVariant }>
) => {
  return <TInput, TOutput, TExpected = TOutput>(
    evalName: string,
    opts: Evaluhealth.RunnerOpts<TInput, TOutput, TExpected, TVariant>
  ) => {
...
const resolveData = async <TInput, TExpected>(
  datasetFunction: Evaluhealth.DataShapeAsyncResolver<TInput, TExpected>
): Promise<Result<Evaluhealth.DataShape<TInput, TExpected>[], Error>> => {
  try {
    return {
      success: true,
...
function registerEvaluhealth<TInput, TOutput, TExpected>(
  evalName: string,
  opts: Evaluhealth.RunnerOpts<TInput, TOutput, TExpected>,
  vitestOpts: {
    modifier?: "only" | "skip";
  ...
  const datasetPromise: Promise<
    Result<Evaluhealth.DataShape<TInput, TExpected>[], Error>
  > =
