...
import EvaluhealthReporter from "./reporter.js";
import { createServer } from "./server.js";
import type { Evaluhealth } from "./types.js";
import { createSqliteStorage } from "./storage/sqlite.js";
import { loadEvaluhealthConfig } from "./config.js";
...
const exportResultsToJSON = async (opts: {
  ...
  const outputData: Evaluhealth.Exported.Output = {
    run: {
      id: latestFullRun.id,
...
/**
 * Run Evaluhealth programmatically via the Node API.
 *
 ...
 * ```
 */
export const runEvaluhealth = async (opts: {
  ...
  const config = await loadEvaluhealthConfig(cwd);
  ...
  process.env.EVALUHEALTH_REPORT_TRACES = "true";
  ...
  const vitest = await createVitest(
    ...
    },
    {
      plugins: [
        {
          name: "evaluhealth-config-plugin",
          // Everything inside this config CAN be overridden by user's vite.config.ts
          // EXCEPT when evaluhealth.config.ts explicitly sets values - those override vite.config.ts
          ...
          config(config: any) {
            config.test ??= {};
            // If evaluhealth.config.ts specifies these values, override user's vite.config.ts
          ...
          configResolved(config: any) {
            if (opts.configDebugMode) {
              const debugMessage = `[Evaluhealth Config Debug] testTimeout: ${config.test?.testTimeout}, maxConcurrency: ${config.test?.maxConcurrency}\n`;
...
/**
 * @deprecated Use `runEvaluhealth` instead. This export will be removed in a future version.
 */
export const runVitest = runEvaluhealth;
