...
export const handleWebsockets = (server: fastify.FastifyInstance) => {
  const websocketListeners = new Map<
    string,
    (event: Evaluhealth.ServerState) => void
  >();

  let currentState: Evaluhealth.ServerState = {
    type: "idle",
  };
  ...
  return {
    updateState: (newState: Evaluhealth.ServerState) => {
...
export const createServer = (opts: { storage: Evaluhealth.Storage }) => {
  ...
  server.get<{
    ...
    ): Evaluhealth.SDK.GetMenuItemsResultEval => {
  ...
  server.route<{
    ...
    Reply: Evaluhealth.SDK.GetEvalByNameResult;
  }>({
    method: "GET",
    url: "/api/eval",
    schema: {
  ...
  server.route<{
    ...
    Reply: Evaluhealth.SDK.GetResultResult;
  }>({
    method: "GET",
    url: "/api/eval/result",
    schema: {
